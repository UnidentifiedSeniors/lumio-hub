...
function Collection() {
  const { user } = useAuth();
  const { champions, traits } = useCatalog();
  ...
  const filteredOwnedChampions = useMemo(() => {
    const championsToShow = ownedChampions.filter((champion) => {
      const championTraits = getChampionTraits(champion);
      const searchableDetails = [champion.name, ...championTraits].filter(Boolean).join(" ").toLowerCase();
  ...
  }, [ownedChampions, query, sortBy, traitFilter]);
  ...
  const hasActiveControls = Boolean(search || traitFilter !== "all" || sortBy !== "recent");
  ...
  return (
    <Layout>
      ...
      <section className="collection-summary" aria-label="Collection summary">
        ...
        <div><span>Tracked traits</span><strong>{availableTraits.filter((trait) => trait !== "Standard").length}</strong></div>
      </section>

      <section className="collection-controls" aria-label="Collection controls">
        <label className="collection-search">
          <span>Search</span>
          <input onChange={(event) => setSearch(event.target.value)} placeholder="Champion or trait" type="search" value={search} />
        </label>
