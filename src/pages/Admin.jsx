...
function Admin() {
  ...
  const loadAdminData = useCallback(async () => {
    ...
    const [metricsResult, adsResult, marketResult, memberResult, tradeResult, auditResult] = await Promise.all([
      ...
      supabase.from("marketplace_listings").select("id, name, trait, note, updated_at, discord_username, discord_display_name").order("updated_at", { ascending: false }).limit(8),
