...
function CatalogPickerDialog({ getItemMeta, items, kind, onChoose, onClose, selectedValue, title }) {
  ...
  const pickerItems = useMemo(() => {
    ...
    const filteredChoices = !normalizedQuery
      ? choices
      : choices.filter((item) => [item.name, isTraitPicker ? item.rarity : null, traitEffectSummary(item), getItemMeta?.(item)]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(normalizedQuery)));

    return [...filteredChoices].sort((left, right) => {
      if (left.isStandard) return -1;
      if (right.isStandard) return 1;
    ...
    <div className="modal-overlay catalog-picker-overlay" role="presentation">
      <section aria-modal="true" className="trade-modal catalog-picker-modal" role="dialog" aria-labelledby="catalog-picker-title">
        <header className="catalog-picker-header">
          <div>
            <p className="eyebrow">{isTraitPicker ? "Trait catalog" : "Champion catalog"}</p>
        ...
        {pickerItems.length ? (
          <>
            <p className="catalog-picker-result-count" aria-live="polite">{pickerItems.length} {isTraitPicker ? "trait" : "champion"}{pickerItems.length === 1 ? "" : "s"}</p>
                ...
                return isTraitPicker ? (
                  <button aria-pressed={selected} className={`catalog-picker-card trait-picker-card${selected ? " is-selected" : ""}`} key={item.name} onClick={() => selectItem(item)} type="button">
                    ...
                    <span className="catalog-picker-card-copy">
                      ...
                      <span className="catalog-picker-card-description">{traitEffectSummary(item)}</span>
